...
6. On the listing page display the number of comments at every issue! (See [custom queries](#!/subjects/webeng/practices/03#custom-queries))

    - Create a Data Transfer Object for the new interface which will be used to store and display the issues data on the listing page. The Issue class is no longer enough for this purpose, because we need to display data (the number of comments) which is not part of the Issue structure.
    - Create a new method inside the IssueRepository to select the number of comments beside the issue data. For this we can use JPQL or a native SQL query.
